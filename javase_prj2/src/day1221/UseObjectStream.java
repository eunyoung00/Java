...
/**
 * UserData를 객체로 생성하여 출력스트림을 통해 HDD File로 내보내고, 읽어들이는 일.
 * @author owner
 */
public class UseObjectStream {
	...
	public void useObjectInputStream() throws IOException,ClassNotFoundException{
		...
		try {
			File file=new File("C:/dev/temp/obj_data.dat");
			//File에 저장된 객체를 얻기위한 스트림 열기 (HDD의 파일을 JVM으로)
			ois=new ObjectInputStream(new FileInputStream(file));
			//스트림을 통해서 직렬화된 객체를 얻어 재조립(unmarshalling)
			UserData ud=(UserData)ois.readObject();
			System.out.println("읽어들인 객체의 값 이름: "+ud.getName()+"/나이: "
						+ud.getAge()+"/몸무게: "+ud.getWeigth());
		}finally {
