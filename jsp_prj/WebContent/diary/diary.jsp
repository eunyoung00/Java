...
.sunTitle{width:100px; height:25px; border:1px solid #CECECE; font-weight:bold; color:#FFFFFF; background-color:#b71540; text-align: center;}
.weekTitle{width:100px; height:25px; border:1px solid #CECECE; text-align: center;}
.satTitle{width:100px; height:25px; border:1px solid #CECECE; font-weight:bold; color:#FFFFFF; background-color:#6a89cc; text-align: center;}
...
#writeFrm{background-color:#FFFFFF; border :1px solid #5c5c5c; box-shadow:5px 5px 5px #444444; padding:10px;}
#readFrm{background-color:#E8F2FE; border:1px solid #0F143C; box-shadow:5px 5px 5px #1C2E6B; padding:10px;}
...
$(function(){
	...
	$("#btnWrite").click(function(){
		if($("#subject").val()==""){
			alert("이벤트 제목은 필수입력입니다.");
			$("#subject").focus();
			return;
		}//end if
		if($("#summernote").val()==""){
			alert("이벤트 내용은 필수입력입니다.");
			$("#summernote").focus();
			return;
		}//end if
		if($("#writer").val()==""){
...
<div id="wrap">
	...
	//오늘 설정
		...
		cal.set(Calendar.DAY_OF_MONTH,1);//요청했을 때 해당 달에 없는 날이 존재한다면 다음달 1일로 넘어가버리므로 무조건 1일로 세팅한다
		//(무조건 다음달)31일이 없는달이 있기 때문에 없는달이 넘어가 2달씩 넘어가게 된다.그래서 다 있는 1일로 설정해 주는것
...
<%-- 	<a href="#diary.jsp?param_month=${nowMonth-1==0?12:nowMonth-1}&param_year=${nowMonth-1==0?nowYear-1 : nowYear}"><img src="images/btn_prev.png" title="이전 월"/></a> --%>
	...
	<!-- <td class="diaryTd">1</td> -->
		...
		MonthVO[] dayEvt=null;//해당일에 글이 존재한다면 저장할 배열.
		...
		for(int tempDay=1; tempDay<33; tempDay++){
			...
			<td class="<%= todayClass %>">
				...
				%>
				<div>
					<%-- <a href="diary.jsp?param_year=${nowYear}&param_month=${nowMonth}&param_day=<%=tempDay%>&pageFlag=write_form"><span class="<%= dayClass%>"> --%>
