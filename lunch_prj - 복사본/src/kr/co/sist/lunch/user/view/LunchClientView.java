...
public class LunchClientView extends JFrame{
	...
	public LunchClientView() {
		...
		dtmLunchList=new DefaultTableModel(lunchColumns, 4) {
			@Override
			public boolean isCellEditable(int row, int column) {//<편집 불가
				return false;
			}//isCellEditable
		};
		jtLunch=new JTable(dtmLunchList){//instance화
			@Override
			public Class<?> getColumnClass(int column) {//Imagefile
		...
		panelOrderNorth.add(new JLabel("주문자 명"));
		panelOrderNorth.add(jtfName);
		panelOrderNorth.add(new Label("전화번호"));
		...
		jtp.addTab("주문 목록 조회", panelOrder);
		//두번째 탭 끝
