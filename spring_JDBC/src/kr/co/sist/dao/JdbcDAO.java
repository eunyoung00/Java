...
public class JdbcDAO {
	...
	/*public JdbcTemplate getJt() throws DataAccessException {
		return jt;
	}*/
	...
	public List<Member> selectAllMember() throws DataAccessException{
		...
		//조회 결과를 Domain에 저장하는 객체를 생성(RowMapper) -DB의 한행 row/record
		RowMapper<Member> rm=new RowMapper<Member>() {

			@Override
			public Member mapRow(ResultSet rs, int idx) throws SQLException {
				Member m=new Member(rs.getInt("num"), rs.getString("name"), rs.getString("img"));
				return m;
			}//mapRow
		};//흔치 않다..!
